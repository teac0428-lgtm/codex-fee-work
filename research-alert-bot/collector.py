...
def process_source(
    ...
    """Process one RSS source and return checked/matched/success/failure counts."""
    ...
    try:
        ...
        for entry in feed.entries:
            ...
            if sent_count + send_successes + send_failures >= MAX_SEND_PER_RUN:
                print("Telegram send skipped: max send limit reached")
                continue

            message = build_alert_message(name, matched_keywords, title, link)
...
def main() -> None:
    ...
    for source in sources:
        checked_entries, matched_entries, source_successes, source_failures = process_source(
            source, keywords, seen_links, send_successes + send_failures
        )
        total_checked_entries += checked_entries
        total_matched_entries += matched_entries
        send_successes += source_successes
        send_failures += source_failures
    ...
    print(f"Telegram sends succeeded: {send_successes}")
    print(f"Telegram sends failed: {send_failures}")
